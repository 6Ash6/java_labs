...
public class GraphicsDisplay extends JPanel {
    ...
    public void displayGraphics(Double[][] GraphicsData, Double[][] OriginalData) {
        ...
        averageData = sum / graphicsData.length;
        ...
        for (Double[] point : graphicsData) {
            ...
            if (point[0] >= viewport[0][0] && point[1] <= viewport[0][1] && point[0] <= viewport[1][0]) {
                ...
                if (i_n == this.selectedMarker) {
                    ...
                    Point2D.Double firstPoint = new Point2D.Double(center.x - 5.5, center.y - 5.5);
                    Point2D.Double secondPoint = new Point2D.Double(center.x + 5.5, center.y - 5.5);
                    Point2D.Double thirdPoint = new Point2D.Double(center.x, center.y + 5.5);

                    canvas.draw(new Line2D.Double(firstPoint.x, firstPoint.y, secondPoint.x, secondPoint.y));
                    canvas.draw(new Line2D.Double(secondPoint.x, secondPoint.y, thirdPoint.x, thirdPoint.y));
                    canvas.draw(new Line2D.Double(thirdPoint.x, thirdPoint.y, firstPoint.x, firstPoint.y));
    ...
    protected void paintLines(Graphics2D canvas) {
        ...
        double step = (maxY - minY) / 100.f;

        for (Double percentage : new Double[]{10.d, 50.d, 90.d}) {
            Double yPos = minY + step * percentage;
            canvas.draw(new Line2D.Double(xyToPoint(minX, yPos),
                    xyToPoint(maxX, yPos)));

            Rectangle2D bounds = axisFont.getStringBounds(percentage.toString(), context);
            Point2D.Double labelPos = xyToPoint(minX, yPos);
            canvas.drawString(String.format("%s%%", percentage), (float) labelPos.getX() + 20,
